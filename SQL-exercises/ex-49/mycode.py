...
def by_dataframe(df:DataFrame, output_path) -> DataFrame:
    new_df = df.withColumn('age', rangeage(df['age']))
    try_to_write_csv(new_df, output_path)
...
def by_sql(session:SparkSession, df: DataFrame, output_path) -> DataFrame:
    ...
    query = """
        SELECT 
            name, 
            surname, 
            CONCAT("[", CAST(FLOOR(AGE / 10) * 10 AS STRING), "-", CAST(FLOOR(AGE/10) * 10 + 9 AS STRING), "]") AS age_range
        FROM 
            users
    """
